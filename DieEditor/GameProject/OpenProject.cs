...
namespace DieEditor.GameProject
{
	...
	class OpenProject
    {
        private static readonly string _applicationDataPath = $@"{Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)}\Diecast\";
		...
		{
            if (System.IO.File.Exists(_projectDataPath))
            {
                ...
                foreach (var project in projects)
                {
                    ...
                    {
                        project.Icon = File.ReadAllBytes($@"{project.ProjectPath}\.die\Icon.png");
                        project.Screenshot = File.ReadAllBytes($@"{project.ProjectPath}\.die\Screenshot.png");
        ...
        static OpenProject()
        {
            try
            {
                if (!System.IO.Directory.Exists(_applicationDataPath))
                {
                    System.IO.Directory.CreateDirectory(_applicationDataPath);
                }
